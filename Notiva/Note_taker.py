...
class AsyncNoteTaker:
    ...
    async def create(
        ...
        try:
            ...
            instance._log.info("note_taker.created", attendee_count=len(attendee_emails))
    ...
    async def _safe_save_segment(self, segment_index: int, result: SegmentOutput, flush_reason: str) -> None:
        try:
            ...
            self._log.error("note_taker.segment_save_failed", segment=segment_index, error=str(exc))
        except Exception as exc:
            self._log.error("note_taker.segment_save_unexpected", segment=segment_index, error=str(exc), error_type=type(exc).__name__)
...
async def create_note_taker(
    ...
    attendee_emails: list[str],
    pool: asyncpg.Pool,
    openrouter_api_key: str = "",       # ← ADDED
) -> AsyncNoteTaker:
    db  = MeetingDB(pool)
    llm = MeetingLLM(api_key=openrouter_api_key)   # ← CHANGED
